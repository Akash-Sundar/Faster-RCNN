...
class RPNHead(pl.LightningModule):
    def __init__(self, batch_tr, batch_val, num_anchors=3, in_channels=256,
        ...
        self.classhead = nn.Sequential(nn.Conv2d(in_channels, num_anchors, kernel_size=(
            1, 1), stride=1, padding='same'),  nn.Sigmoid())
        self.reghead = nn.Sequential(nn.Conv2d(
            in_channels, num_anchors*4, kernel_size=(1, 1), stride=1, padding='same'),  nn.Sigmoid())
    ...
    def forward(self, images):
        feature_pyramid = [v.detach() for v in self.backbone(
            images).values()]  # this has strides [4,8,16,32,64]
        logits = []
        bbox_regs = []
    ...
    def create_anchors(self, aspect_ratio, scale, grid_size, stride):
        ...
        for lev in range(len(scale)):
            anchors_list.append(self.create_anchors_single(
                aspect_ratio[lev], scale[lev], grid_size[lev], stride[lev]))
        return anchors_list
    ...
    def create_anchors_single(self, aspect_ratio, scale, grid_sizes, stride):
        ...
        assert anchors.shape == (
            len(aspect_ratio), grid_sizes[0], grid_sizes[1], 4)
    ...
    def create_batch_truth(self, bboxes_list, indexes, image_shape):
        ...
        for i in range(len(bboxes_list)):
            gclass_i, gcoord_i = self.create_ground_truth(bboxes_list[i].to(device), indexes[i],
                                                          self.anchors_param['grid_size'], self.anchors, image_shape)
        ...
        for lev in range(len(grid_sizes)):
            ...
            for tpe in range(len(anchorsall)):
                ...
                ground_clas = torch.zeros(
                    grid_size[0], grid_size[1]).to(device)
                ground_coord = torch.zeros(
                    4, grid_size[0], grid_size[1]).to(device)
                ...
                ious = []
                idxs_noneg = torch.ones(
                    (grid_size[0], grid_size[1]), dtype=torch.bool).to(device)
                for nbo in range(len(bboxes)):
                    boxB = bboxes[nbo].clone()
                    temp = torchvision.ops.box_iou(torch.unsqueeze(
                        boxB, 0), anchors.reshape(-1, 4)).reshape(grid_size[0], grid_size[1])
                    ious.append(temp)
                    idxs_noneg = torch.logical_and(
                        idxs_noneg, torch.logical_and((temp > 0.3), (temp < 0.7)))
                ...
                for nbo in range(len(bboxes)):
                    ...
                    boxBall = boxB.repeat(
                        grid_size[1], grid_size[0], 1).T.float()

                    # Encoding
                    tx = (boxBall[0, :, :] - anchorswh[0, :, :]
                          ) / anchorswh[2, :, :]
                    ty = (boxBall[1, :, :] - anchorswh[1, :, :]
                          ) / anchorswh[3, :, :]
                    ...
                    ground_coord[:, ious[nbo] >=
                                 0.7] = boxBall[:, ious[nbo] >= 0.7]
                    maxiou = ious[nbo].max().item() - 0.0001
                    ground_clas[ious[nbo] >= maxiou] = 1
                    ground_coord[:, ious[nbo] >=
                                 maxiou] = boxBall[:, ious[nbo] >= maxiou]
                    ground_clas[ious[nbo] < 0.3] = 0
                    ground_coord[:, ious[nbo] <
                                 0.3] = boxBall[:, ious[nbo] < 0.3]*0
    ...
    def loss_class(self, p_out, n_out):
        loss_c = self.bceloss(n_out, torch.zeros((n_out.shape)).to(
            device)) + self.bceloss(p_out, torch.ones((p_out.shape)).to(device))
    ...
    def compute_loss(self, clas_out, regr_out, targ_clas, targ_regr, l=5, effective_batch=150):
        ...
        if len(pos_idx) > (effective_batch/2):
            ...
            rand_idx = torch.randperm(len(neg_idx))[
                :effective_batch - len(pos_idx)]
            sampled_neg_idx = neg_idx[rand_idx]
    ...
    def training_step(self, batch, batch_idx):
        ...
        gt, ground_coord = self.create_batch_truth(
            bbox, index, images.shape[-2:])

        regr_out, clas_out, _ = output_flattening(
            regout, logits, self.get_anchors())
        targ_regr, targ_clas, _ = output_flattening(
            ground_coord, gt, self.get_anchors())
    ...
    def configure_optimizers(self):
        optim = torch.optim.SGD(
            self.parameters(), lr=0.01, momentum=0.9, weight_decay=0.0001)
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(
            optim, [10, 20, 30], gamma=0.1)
    ...
    def topK_plotter(self, images, bbbox, topK=20):
        ...
        flatten_bbox, flatten_clas, flatten_anchors = output_flattening(
            reg_bbox, logits, self.get_anchors())
        decoded_coord = output_decoding(
            flatten_bbox, flatten_anchors).to(device)
        ...
        for elem in topK_bbox:
            ...
            rect = Rectangle((coord[0], coord[1]), coord[2] -
                             coord[0], coord[3]-coord[1], fill=False, color='r')
            ax.add_patch(rect)
        for box in bbbox:
            rect = Rectangle((box[0], box[1]), box[2]-box[0],
                             box[3]-box[1], fill=False, color='b', linewidth=2)
            ax.add_patch(rect)
    ...
    def nms_plotter(self, images, bbbox, keep_num_preNMS, keep_num_postNMS):
        logits, reg_bbox = self.forward(images.float())
        nms_clas, nms_prebox = self.postprocessImg(
            logits, reg_bbox, 0.5, keep_num_preNMS, keep_num_postNMS)
        ...
        for elem in nms_prebox:
            ...
            col = 'r'
            rect = Rectangle((coord[0], coord[1]), coord[2] -
                             coord[0], coord[3]-coord[1], fill=False, color=col)
            ax.add_patch(rect)
        for box in bbbox:
            rect = Rectangle((box[0], box[1]), box[2]-box[0],
                             box[3]-box[1], fill=False, color='b', linewidth=2)
    ...
    def postprocessImg(self, mat_clas, mat_coord, IOU_thresh, keep_num_preNMS, keep_num_postNMS):
        ...
        flatten_bbox, flatten_clas, flatten_anchors = output_flattening(
            mat_coord, mat_clas, self.get_anchors())
        decoded_coord = output_decoding(
            flatten_bbox, flatten_anchors).to(device)
    ...
    def IoU_NMS(self, tar1, tar2):
        ...
        Area_of_int = torch.maximum(torch.Tensor([0]).to(device), xr_int - xl_int) * \
            torch.maximum(torch.Tensor([0]).to(device), yr_int - yl_int)
        Area_of_union = (torch.abs(tar1[2] - tar1[0]) * torch.abs(tar1[3] - tar1[1])) + \
            (torch.abs(tar2[2] - tar2[0]) *
             torch.abs(tar2[3] - tar2[1])) - Area_of_int
